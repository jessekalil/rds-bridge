...
func cmdEnv(args []string) error {
	...
	if db := arg(positionals, 1); db != "" {
		vars = append(vars, [2]string{"DB_DATABASE", db})
	}
	vars = append(vars,
		[2]string{"DB_PORT", strconv.Itoa(t.ListenPort)},
		[2]string{"DB_SSL_REJECT_UNAUTHORIZED", "false"},
		[2]string{"DB_READ_HOST", "127.0.0.1"},
		[2]string{"DB_WRITE_HOST", "127.0.0.1"},
		[2]string{"DB_USER", t.Local.User},
		[2]string{"DB_PASSWORD", t.Local.Password},
	)
	for _, kv := range vars {
